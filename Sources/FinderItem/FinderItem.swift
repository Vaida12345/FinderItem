...
/// ```
///
/// You would still need to call ``tryAccessSecurityScope()`` before and after accessing the file.
///
/// - Experiment: It seems you can only call ``tryAccessSecurityScope()`` on the *original* file, not the ones derived using, for example, ``appending(path:directoryHint:)``. This would mean that to access a child folder, you need to access the security scope of its parent.
///
/// Certain folders are write-only, for example, even with `com.apple.security.files.downloads.read-write`, there is no way to access the contents of Downloads folder created by ``downloadsDirectory``, you need to use a dialog to ask for permission. Then, you can access the contents by persisting its bookmark data.
///
